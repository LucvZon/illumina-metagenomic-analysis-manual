...
singularity exec \
  ...
  python /prepare_project.py \
    ...
    --diamond-db {diamond_database} \
    # Running kaiju is OPTIONAL!
    # Omit --kaiju-db argument if you want to skip running Kaiju
    --kaiju-db {kaiju_database} \
    -t {threads}
```
...
-   `{diamond_database}`: Absolute path pointing to your diamond database (.dmnd).
-   `{kaiju_database}`: Absolute path pointing to your Kaiju database directory. Must contain a `kaiju_db_*.fmi`, `nodes.dmp`, and `names.dmp` file.
