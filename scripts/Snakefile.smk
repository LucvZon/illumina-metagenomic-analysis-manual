...
STUDY_NAME = "Bangladesh_CSF_metagenome_run3"

rule all:
    input:
        expand("result/{sample}/mapping/annotated_idxstats.tsv", sample = FILES),
        expand("result/{sample}/mapping/unannotated_idxstats.tsv", sample = FILES),
        expand("result/{sample}/mapping/viral_idxstats.tsv", sample = FILES),
        expand("result/{sample}/mapping/annotated_contigs.fasta", sample = FILES),
        expand("result/{sample}/mapping/unannotated_contigs.fasta", sample = FILES),
        expand("result/{sample}/mapping/viral_contigs.fasta", sample = FILES)
...
rule filter_host:
    ...
    threads: 4
    params:
        reference = "/mnt/viro0002/workgroups_projects/Bioinformatics/DB/HG38/GCF_000001405.26_GRCh38_genomic.fna.gz"
    shell:
        """
...
rule aggregate_contigs:
    input:
        expand("result/{sample}/assembly/contigs_renamed.fasta", sample = FILES)
    threads: 1
    output:
...
rule split_annotation_files:
    ...
    threads: 1
    output:
        expand("result/{sample}/annotation/diamond_output.tsv", sample = FILES)
    shell:
        '''
        mkdir -p tmp_split
