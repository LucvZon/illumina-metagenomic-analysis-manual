...
def main():
    # Allow 4 arguments (no singletons) or 5 arguments (with singletons)
    if len(sys.argv) not in [5, 6]:
        print("Usage: python3 bin_reads.py <kaiju_names.out> <fq1> <fq2> <output_dir> [singleton_fq]")
        sys.exit(1)
    ...
    fq2_path = sys.argv[3]
    out_dir = sys.argv[4]
    singleton_path = sys.argv[5] if len(sys.argv) == 6 else None
    ...
    with open(kaiju_file, 'r') as f:
        for line in f:
            if line.startswith('C'):  
                ...
                if len(parts) >= 4:
                    read_id = parts[1]
                    tax_ranks = [rank.strip() for rank in parts[3].split(';')]
                    ...
                    if len(tax_ranks) >= 7:
                        ...
                        for level, index in target_levels.items():
                            ...
                            if taxon_name and taxon_name != "NA":
                                read_to_taxa[read_id][level] = sanitize_filename(taxon_name)
    ...
    print("Binning Paired-End reads into FASTA files...")
    ...
    try:
        ...
        # Process Singletons (if provided)
        if singleton_path:
            print("Binning Singleton reads into FASTA files...")
            with open_fastq(singleton_path) as fs:
                while True:
                    hs = fs.readline().strip()
                    if not hs: break
                    ss = fs.readline().strip()
                    fs.readline(); fs.readline()

                    read_id = hs.split()[0][1:]
                    ...
                    if read_id in read_to_taxa:
                        for level, taxon in read_to_taxa[read_id].items():
                            ...
                            file_handles[level][taxon].write(f">{hs[1:]}\n{ss}\n")
